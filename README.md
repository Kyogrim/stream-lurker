...
- **Never miss a go-live.** A background scanner checks every few minutes and can open a
  channel the second it starts, with a desktop notification.
...
Picture-in-Picture controls. Streams run muted at low quality by default; the built-in Ghost
Mode suspends decoding entirely for the ones you only want *credited*, not watched.

![Multi-Lurk Grid with four simultaneous streams](docs/screenshots/multi-lurk-grid.png)

### Lurk Stats

Your lurking, quantified: total watch time, sessions, streaks, a six-month activity heatmap,
a full leaderboard, and a per-streamer breakdown behind any name on it.

![Lurk Stats with activity heatmap and leaderboard](docs/screenshots/lurk-stats.png)
...
| **Runs in the tray** | Closing the window keeps the scanner alive in the background. |
...
2. **Add streamers** under **Manage Streamers**. Their order is the watch priority, used when
   per-platform stream limits are reached.
3. **Turn on Auto-Open** on the dashboard and leave it running. Set your default quality and
   per-platform limits under **System Settings**.
